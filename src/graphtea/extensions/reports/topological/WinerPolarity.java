...
public class WinerPolarity implements GraphReportExtension<RenderTable> {
    ...
    public RenderTable calculate(GraphModel g) {
        ...
        double tempVar = (1 / m) * (Math.floor(m / 2));
        double alpha = (1 / m) * (Math.floor(m)) * (1 - tempVar);
        double alpha1 = (m) * (Math.floor(m / 2)) * (1 - tempVar);
        ...
        //v.add(wp.calculate(Utils.createComplementGraph(g)));

        //Das sir prob
        // v.add((n*(n-1)*(n-2)*(n-2)/2)  +  (2*m*m) + ( (n-1.5)*((2*(m-maxDeg)*(n-maxDeg)/(n-2)) - (maxDeg*(n-maxDeg))) ) -(m*((4*n*n)-(19*n)+17)/2)
        //	   - 2*( (maxDeg*maxDeg) +(((2*m)-maxDeg)*((2*m)-maxDeg)/(n-1)) + (2*(n-2)*(maxDeg2-minDeg)*(maxDeg2-minDeg)/((n-1)*(n-1))) )  );
