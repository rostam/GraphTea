...
public class M3NorConjecture implements GraphReportExtension{
    public String getName() {
        return "M3 Nor Conjecture";
    }
    ...
    public Object calculate(GraphModel g) {
        ...
        ret.get(0).add(" M^3_1(G)+M^3_1(Gcomp) ");
        ret.get(0).add(" 1 ");
        ret.get(0).add(" 2 ");
        ...
        //2
        ret.get(1).add(n*Math.pow(n-1,3) - 3*(n-1)*(2*m*(n-1)-maxDeg*maxDeg)
                + 3*(n-1)*((Math.pow(2*m-maxDeg,2)/(n-1))
                           + 2*(n-2)*(Math.pow(maxDeg2-minDeg,2)
                            / Math.pow(n-1,2))));
