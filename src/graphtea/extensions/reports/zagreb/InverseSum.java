...
public class InverseSum implements GraphReportExtension<RenderTable> {
 ...
 //    v.add( (alpha2*(minDeg-maxDeg)*(maxDeg-minDeg)*(maxDeg+minDeg)/(2*m*maxDeg*minDeg)));
      ...
      //   v.add( ((H*M12)/(2*m)) + (alpha1*(maxDeg-minDeg)*(maxDeg-minDeg)*(maxDeg+minDeg)/(2*m*maxDeg*minDeg)) );
         // Bala sir
   //   v.add(((2*maxDeg*maxDeg*maxDeg*minDeg*minDeg*minDeg)/((maxDeg*maxDeg*maxDeg)+(minDeg*minDeg*minDeg)) )*(chi10+(M22/(4*maxDeg*maxDeg*maxDeg*minDeg*minDeg*minDeg))));
        // v.add((H*M12)/(2*m));
       //   v.add(ABC);
