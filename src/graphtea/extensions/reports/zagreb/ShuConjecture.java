...
public class ShuConjecture implements GraphReportExtension{
    ...
    public Object calculate(GraphData gd) {
        ...
        ret.add(new Vector<Object>());
        ret.get(1).add(Mc31+Mc31gc);
        //1
        ret.get(1).add((n-1)*(M21+M21gc)
                     - n*Math.pow(n-1,3)*Math.pow(2,-2));
        //2
        ret.get(1).add(((n-1)/n)*(2*m*n*(n-1)
                    - Math.pow(2*m - n + Math.sqrt(2*m*Mm11),2)));
        //3
        ret.get(1).add((n-1)*(M21+M21gc) - n*Math.pow(n-1,3));
        //4
        ret.get(1).add(2*m*(n-1)*(n-maxDeg-minDeg-1)
                + (n-1)*((n-a-b)*(maxDeg-minDeg-1) + n*maxDeg*minDeg));
